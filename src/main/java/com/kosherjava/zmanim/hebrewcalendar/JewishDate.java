...
/**
 * The JewishDate is the base calendar class that maintains a Jewish date together with an absolute date and day of
 * week. Gregorian dates are derived from the absolute date as needed, and exposed via the Java time APIs.
 * This class does not have a concept of a time of day. Please note that the calendar does not currently support dates
 * prior to 1/1/1 Gregorian. Also keep in mind that the Gregorian calendar started on October 15, 1582, so any
 * calculations prior to that are suspect (at least from a Gregorian perspective). While 1/1/1 Gregorian and forward
 * are technically supported, any calculations prior to <a href="http://en.wikipedia.org/wiki/Hillel_II">Hillel II's
 * (Hakatan's</a>) calendar (4119 in the Jewish Calendar / 359 CE Julian as recorded by
 * <a href="http://en.wikipedia.org/wiki/Hai_Gaon">Rav Hai Gaon</a>) would be just an approximation.
 *
...
public class JewishDate implements Comparable<JewishDate>, Cloneable {
    ...
    /** The absolute day count since January 1, 0001 Gregorian. */
    private int absDate;
    ...
    private static LocalDate absDateToDate(int absDate) {
        ...
        while (absDate > gregorianDateToAbsDate(year, month, YearMonth.of(year, month).lengthOfMonth())) {
            month++;
        }
    ...
    private static int gregorianDateToAbsDate(int year, int month, int dayOfMonth) {
        ...
        for (int m = month - 1; m > 0; m--) {
            absDate += YearMonth.of(year, m).lengthOfMonth(); // days in prior months of the year
        }
     ...
     * month &lt; 1 or &gt; 12 (or 13 on a {@link #isJewishLeapYear(int) leap year}), the day of month is &lt; 1 or
     * &gt; 30, an hour &lt; 0 or &gt; 23, a minute &lt; 0 or &gt; 59 or <em>chalakim</em> &lt; 0 or &gt; 17.
     * For larger a larger number of <em>chalakim</em> such as 793 (TaShTzaG) break the <em>chalakim</em> into minutes
     * (18 <em>chalakim</em> per minutes, so it would be 44 minutes and 1 <em>chelek</em> in the case of 793 /
     * <em>TaShTzaG</em>).
     *
     * @param year
     *            the Jewish year to validate.
     * @param month
     ...
     *             if a month &lt; 1 or &gt; 12 (or 13 on a leap year), the day of month is &lt; 1 or &gt; 30,
     *             an hour &lt; 0 or &gt; 23, a minute &lt; 0 or &gt; 59 or <em>chalakim</em> &lt; 0 or &gt; 17.
     *             For larger a larger number of <em>chalakim</em> such as 793 (<em>TaShTzaG</em>) break the
     *             <em>chalakim</em> into minutes (18 <em>chalakim</em> per minutes, so it would be 44 minutes and
     *             1 <em>chelek</em> in the case of 793 (<em>TaShTzaG</em>).
     */
    private static void validateJewishDate(int year, int month, int dayOfMonth, int hours, int minutes, int chalakim) {
        validateJewishYear(year);
        ...
        if (dayOfMonth < 1 || dayOfMonth > 30) {
            // Month-specific overflow is normalized by setJewishDate()
    ...
    private static void validateJewishYear(int year) {
        if (year < 1) {
            throw new IllegalArgumentException("A Jewish year of less than 1 can't be set. " + year + " is invalid.");
        }
    }
     ...
     */
    private void setAbsDate(int absDate) {
        if (absDate <= 0) {
            throw new IllegalStateException("Dates before January 1, 1 Gregorian are not supported.");
        }
        ...
        if (absDate < jewishDateToAbsDate(jewishYear, NISSAN, 1)) {
            jewishMonth = TISHREI;// Start at Tishri
        } else {
            jewishMonth = NISSAN;// Start at Nissan
        }
        while (absDate > jewishDateToAbsDate(jewishYear, jewishMonth, getDaysInJewishMonth())) {
            jewishMonth++;
        }
    ...
    /**
     * A constructor that initializes the date to the {@link java.time.ZonedDateTime ZonedDateTime} parameter.
     *
     * @param zonedDateTime
     *            the <code>ZonedDateTime</code> to set the calendar to
     * @throws IllegalStateException
     *             if the resulting date is before January 1, 1 Gregorian
     */
    ...
    public void setLocalDate(LocalDate localDate) {
        ...
        setAbsDate(gregorianDateToAbsDate(localDate.getYear(), localDate.getMonthValue(), localDate.getDayOfMonth()));
    }



    /**
     * Sets the Jewish date and updates the derived absolute date and day of week accordingly.
     *
     * @param year
     *            the Jewish year. The year can't be negative
     * @param month
     ...
     *            has 29 days, the day will be set as 29.
     * @throws IllegalArgumentException
     *             if a month &lt; 1 or &gt; 12 (or 13 on a leap year) or the day of month is &lt; 1 or &gt; 30 is passed in
     * @throws IllegalStateException
     *             if the resulting date is before January 1, 1 Gregorian
     */
     ...
     *
     * @throws IllegalArgumentException
     *             if a month &lt; 1 or &gt; 12 (or 13 on a leap year), the day of month is &lt; 1 or &gt; 30, an hour
     *             &lt; 0 or &gt; 23, a minute &lt; 0 &gt; 59 or <em>chalakim</em> &lt; 0 &gt; 17. For larger a larger
     *             number of <em>chalakim</em> such as 793 (<em>TaShTzaG</em>) break the <em>chalakim</em> into minutes
     *             (18 <em>chalakim</em> per minutes, so it would be 44 minutes and 1 <em>chelek</em> in the case of
     *             793 (<em>TaShTzaG</em>).
     * @throws IllegalStateException
     *             if the resulting date is before January 1, 1 Gregorian
     */
    public void setJewishDate(int year, int month, int dayOfMonth, int hours, int minutes, int chalakim) {
        ...
        setAbsDate(jewishDateToAbsDate(year, month, dayOfMonth));
    }
    ...
    /**
     * Rolls the date forward by one day.
     *
     * @see #back()
     */
    public void forward() {
        setAbsDate(absDate+1);
    }

    /**
     * Forwards the Jewish date by the number of months passed in.
     * FIXME: Deal with forwarding a date such as 30 Nissan by a month. 30 Iyar does not exist. This should be dealt
     * with similar to the way that the Java Calendar behaves.
     *
     * @throws IllegalArgumentException if the amount is less than 1
     * @param amount the number of months to roll the month forward
     */
    public void forwardJewishMonth(int amount) {
        if (amount < 1) {
        ...
        for (int i = 0; i < amount; i++) {
            if (currentMonth == ELUL) {
                ...
                currentYear++;
            } else if ((! isJewishLeapYear(currentYear) && currentMonth == ADAR)
                    || (isJewishLeapYear(currentYear) && currentMonth == ADAR_II)){
                currentMonth = NISSAN;
            } else {
                currentMonth++;
            }
        }
        safeSetJewishYearAndMonth(currentYear,currentMonth);
    }
    /**
     * Forwards the Jewish date by the number of years passed in while keeping the current Jewish month when possible.
     *
     * @throws IllegalArgumentException if the amount is less than 1
     * @param amount the number of years to move forward
     */
    public void forwardJewishYear(int amount) {
        if (amount < 1) {
            throw new IllegalArgumentException("the amount of years to forward has to be greater than zero.");
        }
        safeSetJewishYearAndMonth(getJewishYear() + amount,getJewishMonth());
    }
    private void safeSetJewishYearAndMonth(int year, int month){
        validateJewishYear(year);
        if (month > getLastMonthOfJewishYear(year)) {
            month = getLastMonthOfJewishYear(year);
        }
        int max = getDaysInJewishMonth(month,year);
        setJewishDate(year, month, Math.min(jewishDay, max));
    }

    /**
     * Rolls the date back by one day.
     *
     * @see #forward()
     */
    public void back() {
    ...
    public boolean equals(Object object) {
        ...
        return absDate == jewishDate.getAbsDate();
    }
    ...
    public int compareTo(JewishDate jewishDate) {
        return Integer.compare(absDate, jewishDate.getAbsDate());
    }
    ...
    public void setJewishYear(int year) {
        safeSetJewishYearAndMonth(year, jewishMonth);
    }
