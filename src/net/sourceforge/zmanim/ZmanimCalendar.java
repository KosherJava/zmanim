 ...
 * {@link AstronomicalCalculator} documentation. <h2>
...
public class ZmanimCalendar extends AstronomicalCalendar {
	 ...
	 * before {@link #getSunrise sunrise} in Jerusalem on the equinox is
	 ...
	 * changed via the {@link #setCandleLightingOffset(double)} and retrieved by
	 * the {@link #getCandleLightingOffset()}.
	 */
	private double candleLightingOffset = 18;

	/**
	 * Returns <em>tzais</em> (nightfall) when the sun is 8.5&deg; below the
	 * western geometric horizon (90&deg;) after {@link #getSunset sunset}. For
	 * information on the source of this calculation see
	 * {@link #ZENITH_8_POINT_5}.
	 * 
	 * @return The <code>Date</code> of nightfall. If the calculation can't be
	 ...
	 * below the eastern {@link #GEOMETRIC_ZENITH geometric horizon} before
	 * {@link #getSunrise sunrise}. For more information the source of 16.1&deg;
	 * see {@link #ZENITH_16_POINT_1}.
	 * 
	 * @see net.sourceforge.zmanim.ZmanimCalendar#ZENITH_16_POINT_1
	 * @return The <code>Date</code> of dawn. If the calculation can't be
	 *         computed such as northern and southern locations even south of
	 *         the Arctic Circle and north of the Antarctic Circle where the sun
	 *         may not reach low enough below the horizon for this calculation,
	 *         a null will be returned. See detailed explanation on top of the
	 ...
	 * @return the <code>Date</code> of chatzos. If the calculation can't be
	 *         computed such as in the Arctic Circle where there is at least one
	 *         day where the sun does not rise, and one where it does not set, a
	 ...
	 * 
	 * @return the <code>Date</code> of Solar Midnight (chatzos layla). If the
	 ...
	 * <em>{@link #getShaahZmanisGra() shaos zmaniyos}</em> (solar hours) after
	 * {@link #getSeaLevelSunrise() sea level sunrise} based on the opinion of
	 * the <em>GR"A</em> and the <em>Baal Hatanya</em> that the day is
	 * calculated from sunrise to sunset. This returns the time 3 *
	 * {@link #getShaahZmanisGra()} after {@link #getSeaLevelSunrise() sea level
	 * sunrise}.
	 * 
	 ...
	 * <em>alos</em> being 72 minutes before {@link #getSunrise() sunrise}. This
	 ...
	 * 
	 * @return candle lighting time. If the calculation can't be computed such
	 ...
	 * This returns the time 4 * {@link #getShaahZmanisGra()} after
	 * {@link #getSeaLevelSunrise() sea level sunrise}.
	 * 
	 * @see net.sourceforge.zmanim.ZmanimCalendar#getShaahZmanisGra()
	 * @return the <code>Date</code> of the latest zman tefilah. If the
	 *         calculation can't be computed such as in the Arctic Circle where
	 *         there is at least one day a year where the sun does not rise, and
	 *         one where it does not set, a null will be returned. See detailed
	 *         explanation on top of the {@link AstronomicalCalendar}
	 ...
	 * until <em>{@link #getMinchaKetana() mincha ketana}</em> while the
	 * <em>Ra"sh,
	 ...
	 * calculated as 6.5 {@link #getShaahZmanisGra() sea level solar hours}
	 * after {@link #getSeaLevelSunrise() sea level sunrise}. This calculation
	 * is calculated based on the opinion of the <em>GR"A</em> and the
	 * <em>Baal Hatanya</em> that the day is calculated from sunrise to sunset.
	 * This returns the time 6.5 {@link #getShaahZmanisGra()} after
	 * {@link #getSeaLevelSunrise() sea level sunrise}.
	 * 
	 ...
	 * 9.5 {@link #getShaahZmanisGra() sea level solar hours} after
	 * {@link #getSeaLevelSunrise() sea level sunrise}. This calculation is
	 * calculated based on the opinion of the <em>GR"A</em> and the
	 * <em>Baal Hatanya</em> that the day is calculated from sunrise to sunset.
	 * This returns the time 9.5 * {@link #getShaahZmanisGra()} after
	 * {@link #getSeaLevelSunrise() sea level sunrise}.
	 * 
	 * @see #getShaahZmanisGra()
	 * @see #getMinchaGedola()
	 ...
	 * {@link #getTemporalHour(Date, Date) temporal hour}) according to the
	 ...
	 * the <em>Baal Hatanya</em> that the day runs from
	 * {@link #getSeaLevelSunrise() sunrise} to {@link #getSeaLevelSunrise
	 * sunset}. The calculations are based on a day from
	 * {@link #getSeaLevelSunrise() sea level sunrise} to
	 * {@link #getSeaLevelSunset() sea level sunset}. The day is split into 12
	 * equal parts each part with each one being a <em>shaah zmanis</em>. This
	 * method is similar to {@link #getTemporalHour}, but all calculations are
	 * based on a sealevel sunrise and sunset. For an explanation as to why
	 * elevation is not included in this calculation, see Zmanim Kehilchasam 2nd
	 * Edition by Rabbi Dovid Yehuda Burstein, Jerusalem, 2007. the explanation
	 * and detailed sources for not using elevation for anything besides sunrise
	 * and sunset can be found in <a
	 * href="http://www.worldcat.org/oclc/659793988">Zmanim Kehilchasam (second
	 * edition published in 2007)</a> by Rabbi Dovid Yehuda Bursztyn chapter 2
	 * (pages 186-187).
	 * 
	 * @return the <code>long</code> millisecond length of a
	 *         <em>shaah zmanis</em> calculated from
	 *         {@link #getSeaLevelSunrise() sea level sunrise} to
	 *         {@link #getSeaLevelSunset sea level sunset}. If the calculation
	 *         can't be computed such as in the Arctic Circle where there is at
	 *         least one day a year where the sun does not rise, and one where
	 *         it does not set, {@link Long#MIN_VALUE} will be returned. See
	 *         detailed explanation on top of the {@link AstronomicalCalendar}
	 *         documentation.
