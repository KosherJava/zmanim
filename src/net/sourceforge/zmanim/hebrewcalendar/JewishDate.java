 ...
 * {@link java.util.GregorianCalendar} instance along with the corresponding
 * Jewish date. This class can use the standard Java Date and Calendar classes
 * for setting and maintaining the dates, but it does not subclass these classes
 * or use them internally in any calculations. This class also does not have a
 * concept of a time (which the Date class does). Please note that the calendar
 * does not currently support dates prior to 1/1/1 Gregorian. Also keep in mind
 * that the Gregorian calendar started on October 15, 1582, so any calculations
 * prior to that are suspect (at least from a Gregorian perspective). While
 * 1/1/1 Gregorian and forward are technically supported, any calculations prior
 * to <a
 * href="http://en.wikipedia.org/wiki/Hillel_II">Hillel II's (Hakatan's</a>)
 * calendar (4119 in the Jewish Calendar / 359 CE Julian as recorded by
 * <a href="http://en.wikipedia.org/wiki/Hai_Gaon">Rav Hai Gaon</a>) would be
 * just an approximation.
 *
 * This open source Java code was written by
 * <a href="http://www.facebook.com/avromf">Avrom Finkelstien</a> from his C++
 * code. It was refactored to fit the KosherJava Zmanim API with simplification
 * of the code, enhancements and some bug fixing.
 *
 ...
 * code</a> in
 * <a href="http://www.calendarists.com">Calendrical Calculations</a> by Nachum
 * Dershowitz and Edward M. Reingold, Software-- Practice &amp; Experience, vol.
 * 20, no. 9 (September, 1990), pp. 899- 928. Any method with the mark "ND+ER"
 * indicates that the method was taken from this source with minor
 * modifications.
 *
 * If you are looking for a class that implements a Jewish calendar version of
 * the Calendar class, one is available from the <a href="http://site.icu-project.org/"
 * >ICU (International Components for Unicode)</a> project, formerly part of
...
public class JewishDate implements Comparable<JewishDate>, Cloneable {
    ...
    /**
     * Value of the month field indicating Nissan, the first numeric month of
     * the year in the Jewish calendar. With the year starting at
     * {@link #TISHREI}, it would actually be the 7th (or 8th in a {@link #isJewishLeapYear() leap
     * year}) month of the year.
     */
    public static final int NISSAN = 1;

    /**
     * Value of the month field indicating Iyar, the second numeric month of the
     * year in the Jewish calendar. With the year starting at {@link #TISHREI},
     * it would actually be the 8th (or 9th in a {@link #isJewishLeapYear() leap
     * year}) month of the year.
     */
    public static final int IYAR = 2;

    /**
     * Value of the month field indicating Sivan, the third numeric month of the
     * year in the Jewish calendar. With the year starting at {@link #TISHREI},
     * it would actually be the 9th (or 10th in a {@link #isJewishLeapYear() leap
     * year}) month of the year.
     */
    public static final int SIVAN = 3;

    /**
     * Value of the month field indicating Tammuz, the fourth numeric month of
     * the year in the Jewish calendar. With the year starting at
     * {@link #TISHREI}, it would actually be the 10th (or 11th in a {@link #isJewishLeapYear() leap
     * year}) month of the year.
     */
    public static final int TAMMUZ = 4;

    /**
     * Value of the month field indicating Av, the fifth numeric month of the
     * year in the Jewish calendar. With the year starting at {@link #TISHREI},
     * it would actually be the 11th (or 12th in a
     * {@link #isJewishLeapYear() leap year}) month of the year.
     */
    public static final int AV = 5;

    /**
     * Value of the month field indicating Elul, the sixth numeric month of the
     * year in the Jewish calendar. With the year starting at {@link #TISHREI},
     * it would actually be the 12th (or 13th in a {@link #isJewishLeapYear() leap
     * year}) month of the year.
     */
    public static final int ELUL = 6;

    /**
     * Value of the month field indicating Tishrei, the seventh numeric month of
     * the year in the Jewish calendar. With the year starting at this month, it
     * would actually be the 1st month of the year.
     */
    public static final int TISHREI = 7;

    /**
     * Value of the month field indicating Cheshvan/marcheshvan, the eighth
     * numeric month of the year in the Jewish calendar. With the year starting
     * at {@link #TISHREI}, it would actually be the 2nd month of the year.
     */
    public static final int CHESHVAN = 8;

    /**
     * Value of the month field indicating Kislev, the ninth numeric month of
     * the year in the Jewish calendar. With the year starting at
     * {@link #TISHREI}, it would actually be the 3rd month of the year.
     */
    public static final int KISLEV = 9;

    /**
     * Value of the month field indicating Teves, the tenth numeric month of the
     * year in the Jewish calendar. With the year starting at {@link #TISHREI},
     * it would actually be the 4th month of the year.
     */
    public static final int TEVES = 10;

    /**
     * Value of the month field indicating Shevat, the eleventh numeric month of
     * the year in the Jewish calendar. With the year starting at
     * {@link #TISHREI}, it would actually be the 5th month of the year.
     */
    public static final int SHEVAT = 11;

    /**
     * Value of the month field indicating Adar (or Adar I in a
     * {@link #isJewishLeapYear() leap year}), the twelfth numeric month of the
     * year in the Jewish calendar. With the year starting at {@link #TISHREI},
     * it would actually be the 6th month of the year.
     */
    public static final int ADAR = 12;

    /**
     * Value of the month field indicating Adar II, the leap (intercalary or
     * embolismic) thirteenth (Undecimber) numeric month of the year added in
     * Jewish {@link #isJewishLeapYear() leap year}). The leap years are years
     * 3, 6, 8, 11, 14, 17 and 19 of a 19 year cycle. With the year starting at
     * {@link #TISHREI}, it would actually be the 7th month of the year.
     */
    public static final int ADAR_II = 13;

    /**
     * the Jewish epoch using the RD (Rata Die/Fixed Date or Reingold
     * Dershowitz) day used in Calendrical Calculations. Day 1 is January 1,
     * 0001 Gregorian
     */
    ...
    /**
     * Days from the beginning of Sunday till molad BaHaRaD. Calculated as 1
     * day, 5 hours and 204 chalakim = (24 + 5) * 1080 + 204 = 31524
     */
    private static final int CHALAKIM_MOLAD_TOHU = 31524;

    /**
     * A short year where both {@link #CHESHVAN} and {@link #KISLEV} are 29
     * days.
     *
    ...
    /**
     * An ordered year where {@link #CHESHVAN} is 29 days and {@link #KISLEV} is
     * 30 days.
     *
     * @see #getCheshvanKislevKviah()
     * @see HebrewDateFormatter#getFormattedKviah(int)
     ...
     * {@link #setJewishDate(int, int, int, int, int, int)} or
     * {@link #setMoladHours(int)} will have this field populated. A regular
     * JewishDate object will have this field set to 0.
     *
     ...
     *
     * @param moladHours the molad hours to set
     * @see #getMoladHours()
     * @see #getMolad()
     * @see #setJewishDate(int, int, int, int, int, int)
     *
     ...
     * {@link #setJewishDate(int, int, int, int, int, int)} or or
     * {@link #setMoladMinutes(int)} will have these fields populated. A regular
     * JewishDate object will have this field set to 0.
     *
    ...
    /**
     * Sets the molad minutes. The expectation is that the traditional
     * minute-less chalakim will be broken out to minutes and
     * {@link #setMoladChalakim(int) chalakim/parts} , so 793 (TaShTZaG) parts
     * would have the minutes set to 44 and chalakim to 1.
     *
    ...
    /**
     * Sets the molad chalakim/parts. The expectation is that the traditional
     * minute-less chalakim will be broken out to
     * {@link #setMoladMinutes(int) minutes} and chalakim, so 793 (TaShTZaG)
     * parts would have the minutes set to 44 and chalakim to 1.
     *
     * @param moladChalakim the molad chalakim/parts to set
     ...
     * {@link #setJewishDate(int, int, int, int, int, int)} or or
     * {@link #setMoladChalakim(int)} will have these fields populated. A
     * regular JewishDate object will have this field set to 0.
     *
    ...
    /**
     * The month, where 1 == January, 2 == February, etc... Note that this is
     * different than the Java's Calendar class where January ==0
     */
     ...
     *
     * @param month the month. As with other cases in this class, this is
     ...
     * @param month the Gregorian month. Unlike the Java Calendar where January
    ...
    /**
     * Returns if the year is a Jewish leap year. Years 3, 6, 8, 11, 14, 17 and
     * 19 in the 19 year cycle are leap years.
     *
    ...
    /**
     * Returns if the year the calendar is set to is a Jewish leap year. Years
     * 3, 6, 8, 11, 14, 17 and 19 in the 19 year cycle are leap years.
     *
     * @return true if it is a leap year
    ...
    /**
     * Returns the last month of a given Jewish year. This will be 12 on a non
     * {@link #isJewishLeapYear(int) leap year} or 13 on a leap year.
     *
     * @param year the Jewish year.
     ...
     * the Jewish calendar to the mean conjunction of Tishri of the Jewish year.
     *
     * @param year the Jewish year
     * @return the number of days elapsed from prior to the molad Tohu BaHaRaD
     * (Be = Monday, Ha= 5 hours and Rad =204 chalakim/parts) prior to the start
     * of the Jewish calendar, to the mean conjunction of Tishri of the Jewish
     * year. BeHaRaD is 23:11:20 on Sunday night(5 hours 204/1080 chalakim after
     ...
     * <ol>
     * <li>Lo ADU Rosh - Rosh Hashana can't fall on a Sunday, Wednesday or
     * Friday. If the molad fell on one of these days, Rosh Hashana is delayed
     * to the following day.</li>
     * <li>Molad Zaken - If the molad of Tishrei falls after 12 noon, Rosh
     * Hashana is delayed to the following day. If the following day is ADU, it
     * will be delayed an additional day.</li>
     * <li>GaTRaD - If on a non leap year the molad of Tishrei falls on a
     * Tuesday (Ga) on or after 9 hours (T) and 204 chalakim (TRaD) it is
     * delayed till Thursday (one day delay, plus one day for Lo ADU Rosh)</li>
     * <li>BeTuTaKFoT - if the year following a leap year falls on a Monday (Be)
     * on or after 15 hours (Tu) and 589 chalakim (TaKFoT) it is delayed till
     * Tuesday</li>
     * </ol>
     *
     * @param year the year
     ...
     * @return the number of elapsed days in the JewishCalendar adjusted for the
     * 4 dechiyos.
     */
    ...
    /**
     * Returns the number of chalakim (parts - 1080 to the hour) from the
     * original hypothetical Molad Tohu to the year and month passed in.
     *
     * @param year the Jewish year
     * @param month the Jewish month the Jewish month, with the month numbers
     * starting from Nisan. Use the JewishDate constants such as
     * {@link JewishDate#TISHREI}.
     * @return the number of chalakim (parts - 1080 to the hour) from the
     * original hypothetical Molad Tohu
     */
    ...
    /**
     * Returns the number of chalakim (parts - 1080 to the hour) from the
     * original hypothetical Molad Tohu to the Jewish year and month that this
     * Object is set to.
     *
     * @return the number of chalakim (parts - 1080 to the hour) from the
     * original hypothetical Molad Tohu
    ...
    /**
     * Converts the the {@link JewishDate#NISSAN} based constants used by this
     * class to numeric month starting from {@link JewishDate#TISHREI}. This is
     * required for Molad claculations.
     *
     * @param year The Jewish year
    ...
    /**
     * Validates the components of a Jewish date for validity. It will throw an
     * {@link IllegalArgumentException} if the Jewish date is earlier than 18
     * Teves, 3761 (1/1/1 Gregorian), a month &lt; 1 or &gt; 12 (or 13 on a
     * {@link #isJewishLeapYear(int) leap year}), the day of month is &lt; 1 or
     * &gt; 30, an hour &lt; 0 or &gt; 23, a minute &lt; 0 or &gt; 59 or
     * chalakim &lt; 0 or &gt; 17. For larger a larger number of chalakim such
     * as 793 (TaShTzaG) break the chalakim into minutes (18 chalakim per
     * minutes, so it would be 44 minutes and 1 chelek in the case of
     * 793/TaShTzaG).
     *
     * @param year the Jewish year to validate. It will reject any year &lt;=
     * 3761 (lower than the year 1 Gregorian).
     * @param month the Jewish month to validate. It will reject a month &lt; 1
     * or &gt; 12 (or 13 on a leap year) .
     * @param dayOfMonth the day of the Jewish month to validate. It will reject
     * any value &lt; 1 or &gt; 30 TODO: check calling methods to see if there
     * is any reason that the class can validate that 30 is invalid for some
     * months.
     * @param hours the hours (for molad calculations). It will reject an hour
     * &lt; 0 or &gt; 23
     * @param minutes the minutes (for molad calculations). It will reject a
     * minute &lt; 0 or &gt; 59
     * @param chalakim the chalakim/parts (for molad calculations). It will
     * reject a chalakim &lt; 0 or &gt; 17. For larger numbers such as 793
     * (TaShTzaG) break the chalakim into minutes (18 chalakim per minutes, so
     * it would be 44 minutes and 1 chelek in the case of 793/TaShTzaG)
     *
     * @throws IllegalArgumentException if a A Jewish date earlier than 18
     * Teves, 3761 (1/1/1 Gregorian), a month &lt; 1 or &gt; 12 (or 13 on a leap
     * year), the day of month is &lt; 1 or &gt; 30, an hour &lt; 0 or &gt; 23,
     * a minute &lt; 0 or &gt; 59 or chalakim &lt; 0 or &gt; 17. For larger a
     * larger number of chalakim such as 793 (TaShTzaG) break the chalakim into
     * minutes (18 chalakim per minutes, so it would be 44 minutes and 1 chelek
    ...
    /**
     * Validates the components of a Gregorian date for validity. It will throw
     * an {@link IllegalArgumentException} if a year of &lt; 1, a month &lt; 0
     * or &gt; 11 or a day of month &lt; 1 is passed in.
     *
     * @param year the Gregorian year to validate. It will reject any year &lt;
     * 1.
     * @param month the Gregorian month number to validate. It will enforce that
     * the month is between 0 - 11 like a {@link GregorianCalendar}, where
     * {@link Calendar#JANUARY} has a value of 0.
     * @param dayOfMonth the day of the Gregorian month to validate. It will
     * reject any value &lt; 1, but will allow values &gt; 31 since calling
     * methods will simply set it to the maximum for that month. TODO: check
     * calling methods to see if there is any reason that the class needs days
     * &gt; the maximum.
     * @throws IllegalArgumentException if a year of &lt; 1, a month &lt; 0 or
     * &gt; 11 or a day of month &lt; 1 is passed in
    ...
    /**
     * Returns the number of days for the current year that the calendar is set
     * to.
     *
    ...
    /**
     * Returns if Cheshvan is long in a given Jewish year. The method name
     * isLong is done since in a Kesidran (ordered) year Cheshvan is short.
     * ND+ER
     *
     * @param year the year
    ...
    /**
     * Returns if Cheshvan is long (30 days VS 29 days) for the current year
     * that the calendar is set to. The method name isLong is done since in a
     * Kesidran (ordered) year Cheshvan is short.
     *
     * @return true if Cheshvan is long for the current year that the calendar
     * is set to
     * @see #isCheshvanLong()
     */
     ...
     * The method name isShort is done since in a Kesidran (ordered) year Kislev
     * is long. ND+ER
     *
     * @param year the Jewish year
    ...
    /**
     * Returns the Cheshvan and Kislev kviah (whether a Jewish year is short,
     * regular or long). It will return {@link #SHELAIMIM} if both cheshvan and
     * kislev are 30 days, {@link #KESIDRAN} if Cheshvan is 29 days and Kislev
     * is 30 days and {@link #CHASERIM} if both are 29 days.
     *
     * @return {@link #SHELAIMIM} if both cheshvan and kislev are 30 days,
     * {@link #KESIDRAN} if Cheshvan is 29 days and Kislev is 30 days and
     * {@link #CHASERIM} if both are 29 days.
     * @see #isCheshvanLong()
     * @see #isKislevShort()
     */
    ...
    /**
     * Returns the number of days of the Jewish month that the calendar is
     * currently set to.
     *
     * @return the number of days for the Jewish month that the calendar is
     * currently set to.
     */
     ...
     * @param month the Jewish month starting with Nisan. Nisan expects a value
     * of 1 etc till Adar with a value of 12. For a leap year, 13 will be the
     * expected value for Adar II. Use the constants {@link JewishDate#NISSAN}
     * etc.
     * @param dayOfMonth the Jewish day of month. valid values are 1-30. If the
     * day of month is set to 30 for a month that only has 29 days, the day will
     * be set as 29.
    ...
    /**
     * Returns the molad for a given year and month. Returns a JewishDate
     * {@link Object} set to the date of the molad with the
     * {@link #getMoladHours() hours}, {@link #getMoladMinutes() minutes} and {@link #getMoladChalakim()
     * chalakim} set. In the current implementation, it sets the molad time
     * based on a midnight date rollover. This means that Rosh Chodesh Adar II,
     * 5771 with a molad of 7 chalakim past midnight on Shabbos 29 Adar I /
     * March 5, 2011 12:00 AM and 7 chalakim, will have the following values:
     * hours: 0, minutes: 0, Chalakim: 7.
     *
     * @return a JewishDate {@link Object} set to the date of the molad with the {@link #getMoladHours() hours},
     *         {@link #getMoladMinutes() minutes} and
     * {@link #getMoladChalakim() chalakim} set.
     */
    ...
    /**
     * Returns the number of days from the Jewish epoch from the number of
     * chalakim from the epoch passed in.
     *
     * @param chalakim the number of chalakim since the beginning of Sunday
     * prior to BaHaRaD
    ...
    /**
     * Constructor that creates a JewishDate based on a molad passed in. The
     * molad would be the number of chalakim/parts starting at the beginning of
     * Sunday prior to the molad Tohu BeHaRaD (Be = Monday, Ha= 5 hours and Rad
     * =204 chalakim/parts) - prior to the start of the Jewish calendar. BeHaRaD
     * is 23:11:20 on Sunday night(5 hours 204/1080 chalakim after sunset on
     * Sunday evening).
     *
     * @param molad the number of chalakim since the beginning of Sunday prior
     * to BaHaRaD
     */
    ...
    /**
     * Sets the molad time (hours minutes and chalakim) based on the number of
     * chalakim since the start of the day.
     *
     * @param chalakim the number of chalakim since the start of the day.
     */
    ...
    /**
     * returns the number of days from Rosh Hashana of the date passed in, to
     * the full date passed in.
     *
     * @param year the Jewish year
     * @param month the Jewish month
     ...
     * @param jewishMonth the Jewish month. The method expects a 1 for Nissan
     * ... 12 for Adar and 13 for Adar II. Use the constants {@link #NISSAN} ...
     * {@link #ADAR} (or {@link #ADAR_II} for a leap year Adar II) to avoid any
     * confusion.
     * @param jewishDayOfMonth the Jewish day of month. If 30 is passed in for a
     * month with only 29 days (for example {@link #IYAR}, or {@link #KISLEV} in
     * a year that {@link #isKislevShort()}), the 29th (last valid date of the
     * month) will be set
     * @throws IllegalArgumentException if the day of month is &lt; 1 or &gt;
     * 30, or a year of &lt; 0 is passed in.
     */
     ...
     * {@link java.util.Date Date} paremeter.
     *
     * @param date the <code>Date</code> to set the calendar to
     * @throws IllegalArgumentException if the date would fall prior to the
     * January 1, 1 AD
     */
    ...
    /**
     * A constructor that initializes the date to the
     * {@link java.util.Calendar Calendar} paremeter.
     *
     * @param calendar the <code>Calendar</code> to set the calendar to
     * @throws IllegalArgumentException if the {@link Calendar#ERA} is
     * {@link GregorianCalendar#BC}
     */
    ...
    /**
     * A constructor that initializes the date to the
     * {@link java.time.LocalDate LocalDate} paremeter.
     *
     ...
     *
     * @param date the <code>Date</code> to set the calendar to
     * @throws IllegalArgumentException if the date would fall prior to the year
     * 1 AD
     */
    ...
    /**
     * Sets the date based on a {@link java.time.LocalDate LocalDate} object.
     * Modifies the Jewish date as well.
     *
     * @param localDate the <code>LocalDate</code> to set the calendar to
     * @throws IllegalArgumentException if the date would fall prior to the year
    ...
    /**
     * Sets the Gregorian Date, and updates the Jewish date accordingly. Like
     * the Java Calendar A value of 0 is expected for January.
     *
     * @param year the Gregorian year
     * @param month the Gregorian month. Like the Java Calendar, this class
     * expects 0 for January
     * @param dayOfMonth the Gregorian day of month. If this is &gt; the number
     * of days in the month/year, the last valid date of the month will be set
     * @throws IllegalArgumentException if a year of &lt; 1, a month &lt; 0 or
     * &gt; 11 or a day of month &lt; 1 is passed in
     */
    ...
    /**
     * Sets the hidden internal representation of the Gregorian date , and
     * updates the Jewish date accordingly. While public getters and setters
     * have 0 based months matching the Java Calendar classes, This class
     * internally represents the Gregorian month starting at 1. When this is
     * called it will not adjust the month to match the Java Calendar classes.
     *
     * @param year the year
     ...
     *
     * @param year the Jewish year. The year can't be negative
     * @param month the Jewish month starting with Nisan. A value of 1 is
     * expected for Nissan ... 12 for Adar and 13 for Adar II. Use the constants
     * {@link #NISSAN} ... {@link #ADAR} (or {@link #ADAR_II} for a leap year
     * Adar II) to avoid any confusion.
     * @param dayOfMonth the Jewish day of month. valid values are 1-30. If the
     * day of month is set to 30 for a month that only has 29 days, the day will
     * be set as 29.
     * @throws IllegalArgumentException if a A Jewish date earlier than 18
     * Teves, 3761 (1/1/1 Gregorian), a month &lt; 1 or &gt; 12 (or 13 on a leap
     * year) or the day of month is &lt; 1 or &gt; 30 is passed in
     */
     ...
     * be set as 29.
     *
     * @param hours the hour of the day. Used for Molad calculations
     * @param minutes the minutes. Used for Molad calculations
     * @param chalakim the chalakim/parts. Used for Molad calculations. The
     * chalakim should not exceed 17. Minutes should be used for larger numbers.
     *
     * @throws IllegalArgumentException if a A Jewish date earlier than 18
     * Teves, 3761 (1/1/1 Gregorian), a month &lt; 1 or &gt; 12 (or 13 on a leap
     * year), the day of month is &lt; 1 or &gt; 30, an hour &lt; 0 or &gt; 23,
     * a minute &lt; 0 &gt; 59 or chalakim &lt; 0 &gt; 17. For larger a larger
     * number of chalakim such as 793 (TaShTzaG) break the chalakim into minutes
     * (18 chalakim per minutes, so it would be 44 minutes and 1 chelek in the
    ...
    /**
     * Returns a string containing the Jewish date in the form, "day Month,
     * year" e.g. "21 Shevat, 5729". For more complex formatting, use the
     * formatter classes.
     *
     * @return the Jewish date in the form "day Month, year" e.g. "21 Shevat,
     * 5729"
    ...
    /**
     * Rolls the date, month or year forward by the amount passed in. It
     * modifies both the Gregorian and Jewish dates accordingly. If manipulation
     * beyond the fields supported here is required, use the {@link Calendar}
     * class {@link Calendar#add(int, int)} or {@link Calendar#roll(int, int)}
     ...
     * </pre>
     *
     * @param field the calendar field to be forwarded. The must be
     * {@link Calendar#DATE}, {@link Calendar#MONTH} or {@link Calendar#YEAR}
     * @param amount the positive amount to move forward
     * @throws IllegalArgumentException if the field is anything besides
     * {@link Calendar#DATE}, {@link Calendar#MONTH} or {@link Calendar#YEAR} or
    ...
    /**
     * Forward the Jewish date by the number of months passed in. FIXME: Deal
     * with forwarding a date such as 30 Nisan by a month. 30 Iyar does not
     * exist. This should be dealt with similar to the way that the Java
     * Calendar behaves (not that simple since there is a difference between
     * add() or roll().
     *
    ...
    /**
     * Rolls the date back by 1 day. It modifies both the Gregorian and Jewish
     * dates accordingly. The API does not currently offer the ability to
     * forward more than one day t a time, or to forward by month or year. If
     * such manipulation is required use the {@link Calendar} class
     * {@link Calendar#add(int, int)} or {@link Calendar#roll(int, int)} methods
     ...
     *
     * @return the Gregorian month (between 0-11). Like the java.util.Calendar,
     * months are 0 based.
     */
    ...
    /**
     * Returns the Jewish month 1-12 (or 13 years in a leap year). The month
     * count starts with 1 for Nisan and goes to 13 for Adar II
     *
     ...
     * @param month the Gregorian month
     *
     * @throws IllegalArgumentException if a month &lt; 0 or &gt; 11 is passed
     * in
     */
     ...
     *
     * @param year the Gregorian year.
     * @throws IllegalArgumentException if a year of &lt; 1 is passed in
     */
     ...
     *
     * @param dayOfMonth the Gregorian Day of month.
     * @throws IllegalArgumentException if the day of month of &lt; 1 is passed
     * in
     */
     ...
     *
     * @param month the Jewish month from 1 to 12 (or 13 years in a leap year).
     * The month count starts with 1 for Nisan and goes to 13 for Adar II
     * @throws IllegalArgumentException if a month &lt; 1 or &gt; 12 (or 13 on a
     * leap year) is passed in
     */
     ...
     *
     * @param year the Jewish year
     * @throws IllegalArgumentException if a year of &lt; 3761 is passed in. The
     * same will happen if the year is 3761 and the month and day previously set
     * are &lt; 18 Teves (preior to Jan 1, 1 AD)
     */
     ...
     *
     * @param dayOfMonth the Jewish day of month
     * @throws IllegalArgumentException if the day of month is &lt; 1 or &gt; 30
     * is passed in
     */
     ...
     * <a href="http://en.wikipedia.org/wiki/Object_copy#Deep_copy">deep
     * copy</a> of the object.
     *
     * @see Object#clone()
     */
    public Object clone() {
